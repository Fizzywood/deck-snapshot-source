...
- Store crypt recovery secrets only in the separately exported private recovery file. At runtime, pass their rclone-obscured forms through the two exact allowlisted `RCLONE_CRYPT_PASSWORD*` environment fields; never persist production crypt secrets in `rclone.conf` or place them on an argument vector.
...
- Require a matching recovery-material fingerprint acknowledgement before first upload. An acknowledgement is not a key and must never substitute for recovery material.
